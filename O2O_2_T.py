...
# Load dataset
data_path = "./data/O2O_2_T/"
pic_path = "./pic/O2O_2_T/"
model_path = "./model/O2O_2_T/"
...
# 需要打乱数据加上这一行,对行数进行打乱
shuffle_ix = np.random.permutation(np.arange(T.shape[0]))
T = T[shuffle_ix]
wO2 = wO2[shuffle_ix]
wO = wO[shuffle_ix]
...
true_T = np.insert(y_test[0:N_test, :],0,Ma_test,axis=1)
pred_T = np.insert(y_pred[0:N_test, :],0,Ma_test,axis=1)
# save
print("Saving prediction data ...")
np.savetxt(data_path+'test_T_pred.dat', pred_T,fmt='%.6f')
np.savetxt(data_path+'test_T_true.dat', true_T,fmt='%.6f')