...
# Load dataset
data_path = "./data/UTTv_2_rho/"
pic_path = "./pic/UTTv_2_rho/"
model_path = "./model/UTTv_2_rho/"
...
w_total = np.ones((501,1000))
w_total_train = w_total[0:N_train, :]
w_total_valid = w_total[N_train:N_train+N_valid, :]
w_total_test = w_total[-N_test:, :]

input_train = np.concatenate((U_train,T_train,Tv_train), axis=0)
output_train = np.concatenate((wO2_train,wO_train,w_total_train), axis=0)
input_valid = np.concatenate((U_valid,T_valid,Tv_valid), axis=0)
output_valid = np.concatenate((wO2_valid,wO_valid,w_total_valid), axis=0)
input_test = np.concatenate((U_test,T_test,Tv_test), axis=0)
output_test = np.concatenate((wO2_test,wO_test,w_total_test), axis=0)
...
# Compile and Train
model.compile("adam", lr=0.00006, metrics=["mean l2 relative error"])
checker = dde.callbacks.ModelCheckpoint(
    model_path+"model.ckpt", verbose=1, save_better_only=True, period=1000
)
losshistory, train_state = model.train(iterations=500000, callbacks=[checker])
...
train_Tv = np.insert(train_state.X_train[0][2*N_train:3*N_train, :],0,Ma_train,axis=1)
...
valid_Tv = np.insert(train_state.X_test[0][2*N_valid:3*N_valid, :],0,Ma_valid,axis=1)
...
test_Tv = np.insert(input_test[2*N_test:3*N_test, :],0,Ma_test,axis=1)
...
np.savetxt(data_path+'train_Tv.dat', train_Tv,fmt='%.6f')
...
np.savetxt(data_path+'valid_Tv.dat', valid_Tv,fmt='%.6f')
...
np.savetxt(data_path+'test_Tv.dat', test_Tv,fmt='%.6f')
